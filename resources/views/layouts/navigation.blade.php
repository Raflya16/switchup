<nav x-data="{ open: false }" class="bg-white dark:bg-white border-b border-gray-100 dark:border-gray-200">
                ...
                <div class="hidden sm:flex sm:items-center sm:ms-6">
                    @auth
                        <div class="flex items-center space-x-2">
                            <a href="{{ route('messages.inbox') }}" class="relative inline-flex items-center p-2 text-sm font-medium text-center text-gray-500 hover:text-gray-700 rounded-lg focus:outline-none">
                                    ...
                                    <x-slot name="content">
                                        @forelse(auth()->user()->unreadNotifications->take(5) as $notification)
                                            <div class="px-4 py-2 border-b text-sm text-gray-600">
                                                <p class="font-semibold text-gray-800">{{ data_get($notification->data, 'offerer_name') }}</p>
                                                <p>menawarkan barangnya untuk ditukar.</p>
                                                <a href="{{ route('notifications.read', $notification->id) }}" class="text-primary hover:underline">Lihat penawaran</a>
                                            </div>
                                        @empty
                                    ...
                                    <x-slot name="trigger">
                                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                                            <div class="flex items-center mr-3">
                                                <svg class="w-5 h-5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path d="M10 2a8 8 0 100 16 8 8 0 000-16zM6.343 12.343a6 6 0 018.486-8.486l-8.486 8.486zM12 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                                                <span class="ml-1 text-sm font-semibold text-gray-700">{{ auth()->user()->tokens }}</span>
                                            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('home')" :active="request()->routeIs('home')">{{ __('Beranda') }}</x-responsive-nav-link>
            @auth
                <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">{{ __('Barang Saya') }}</x-responsive-nav-link>
                <x-responsive-nav-link :href="route('barters.index')" :active="request()->routeIs('barters.index')">{{ __('Penawaran') }}</x-responsive-nav-link>
            @endauth
            <x-responsive-nav-link :href="route('about')" :active="request()->routeIs('about')">{{ __('Tentang Kami') }}</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('contact')" :active="request()->routeIs('contact')">{{ __('Kontak') }}</x-responsive-nav-link>
        </div>
        @auth
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                <div class="font-medium text-base text-gray-800"><span translate="no">{{ Auth::user()->name }}</span></div>
                <div class="font-medium text-sm text-gray-500">{{ Auth::user()->email }}</div>
                <div class="flex items-center mt-2">
                    <svg class="w-5 h-5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path d="M10 2a8 8 0 100 16 8 8 0 000-16zM6.343 12.343a6 6 0 018.486-8.486l-8.486 8.486zM12 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                    <span class="ml-1 text-sm font-semibold text-gray-700">{{ auth()->user()->tokens }} Tokens</span>
                </div>
            </div>
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link :href="route('profile.edit')">{{ __('Profile') }}</x-responsive-nav-link>
                <x-responsive-nav-link :href="route('tokens.index')">{{ __('Top Up Token') }}</x-responsive-nav-link>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
